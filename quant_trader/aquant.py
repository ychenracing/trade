...
======================================================
本文件默认参数已写入9标的、单票买入上限60%、总仓位上限100%的最终调优结果。
标的：中际旭创、新易盛、天孚通信、澜起科技、兆易创新、雅克科技、联瑞新材、鼎龙股份、华海诚科。
2025-01-02至2026-06-30真实前复权组合回测：总收益率1175.13%，标准全局最大回撤-19.46%。
...
class Indicators:
    ...
    def atr(df: pd.DataFrame, period: int = 20) -> pd.Series:
        """ATR (Average True Range) — 标准 Wilder 初始化和平滑。"""
...
class BaseStrategy:
    ...
    def _calc_shares(self, capital: float, price: float, atr_val: float) -> int:
        ...
        atr_mult = float(self.cfg.get("atr_multiplier", 3))
        if (
    ...
    def on_bar(self, ctx: BarContext) -> Signal | None:
        ...
        if self.position is not None:
            ...
            if pos.units < max_units:
                ...
                base_add_price = pos.last_add_price or pos.entry_price
            ...
            if loc > 0:
                ...
                if prev_close > 0:
                    ...
                    if signal.direction == "buy" and change_pct >= limit_up - 0.001:
                        # 开盘涨停，无法买入 → 丢弃买入信号（不保留，次日条件可能已变）
                        continue
                    if signal.direction == "sell" and change_pct <= limit_down + 0.001:
                        # 开盘跌停，无法卖出 → [FIX R1-1] 卖出信号保留到次日继续尝试
                        unexecuted.append((signal, strategy))
                        continue
