"""Research-only AB5 recovery comparison on the frozen 17-symbol replay window."""
...
def _sell_rebuy_metrics(trades: list[Any], equity_curve: list[dict[str, Any]]) -> dict[str, int]:
    sessions = {str(row["date"]): index for index, row in enumerate(equity_curve)}
...
def run() -> dict[str, Any]:
    ...
    mean_assets = fmean(float(row["assets"]) for row in equity_curve)
    ...
    recovery_blocked_orders = sum(
        int(event.get("ab5_recovery_blocked_orders", 0)) for event in recovery_events
    )
    recovery_blocked_shares = sum(
        int(event.get("ab5_recovery_buy_shares_blocked", 0)) for event in recovery_events
    )
    state_counts: dict[str, int] = defaultdict(int)
    for event in recovery_events:
        state_counts[str(event.get("ab5_recovery_state", "UNANNOTATED"))] += 1
    ...
    metrics = {
        ...
        "recovery_blocked_orders": recovery_blocked_orders,
        "recovery_blocked_shares": recovery_blocked_shares,
        "recovery_state_days": dict(sorted(state_counts.items())),
    }
