#!/usr/bin/env python3
"""Validate and assemble temporary git-backed exact-958 shard evidence."""
...
def _validate_finite_metrics(item: dict[str, Any]) -> None:
    ...
    for key in ("total_return", "max_drawdown", "sharpe", "calmar"):
        try:
            ...
            raise RuntimeError(
                f"missing {key} in {item.get('scenario_id')!r}"
            ) from exc
        if type(value) not in (int, float) or not math.isfinite(value):
            raise RuntimeError(
                f"non-finite/non-numeric {key} in "
                f"{item.get('scenario_id')!r}: {value!r}"
            )
...
def validate_final(
    ...
    expected_provenance = _provenance(_full_plan(), source_sha)
    for field in stress_artifacts.PROVENANCE_FIELDS:
        if candidate.get(field) != expected_provenance[field]:
    ...
    if summary.get("provenance") != {
        ...
        raise RuntimeError("summary/candidate provenance mismatch")
    for field in (
        "artifact_status",
        "acceptance_status",
        "canonical",
        ...
        if summary.get(field) != candidate.get(field):
            raise RuntimeError(f"summary/candidate mismatch: {field}")
    replay_fields = (
        "engine",
        "deployment_policy",
    ...
    if summary.get("replay_contract") != {
        field: candidate[field] for field in replay_fields
    }:
        raise RuntimeError("summary/candidate replay contract mismatch")
