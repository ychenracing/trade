...
- `domain/`、`config/`、`data/`、`indicators/`：领域模型、配置事实源、行情契约和无副作用指标。
...
- 根目录历史模块和命令行文件：仅作为薄兼容出口或入口，不得成为规范包的依赖源。
- `scripts/`：可复现研究与验证命令。
- `tests/`：单元、契约、集成和经济回归测试。

依赖方向、无环导入、私有名称边界和兼容层规模由架构契约测试守卫。
...
CI 使用锁定依赖，并分别覆盖受支持的 Python 版本、完整测试、类型检查、安全审计、依赖审计和精确经济回归。昂贵回测、基准和模拟按 `AGENTS.md` 的 L1–L4 分层验证执行。

## 7. Important Paths

- `quantfusion/`：规范模块化实现。
- `quantfusion/config/`：引擎、组合、风险、路由和股票池的公共配置事实源。
- `scripts/`：批量回测、数据下载、篮子验证和压力工具。
...
按 `AGENTS.md` 先做最小受影响验证，里程碑时扩大到子系统，最终稳定候选再执行完整适用门。Definition of Done：适用检查通过；证据准确记录；未运行项明确标记；没有未解决的正确性、安全、数据完整性或阻断审查问题。
