...
from quantfusion.strategy.trend import DualMAStrategy, TurtleBreakoutStrategy
...
def run_quiet(codes: tuple[str, ...], state: str = "warm") -> dict:
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        return BacktestEngine(2_000_000).run(
...
class PolicyTests(unittest.TestCase):
    ...
    def test_invalid_regime_symbols_are_rejected(self) -> None:
        for symbols in ((), ("300308", "300308"), ("invalid",)):
            with self.subTest(symbols=symbols), self.assertRaises(ValueError):
                PortfolioPolicy(regime_symbols=symbols)
    ...
    def test_date_symbol_side_count_has_explicit_bucket_semantics(self) -> None:
        trades = [
            TradeRecord("300308", strategy, direction, 100, 10.0, date)
        ...
        self.assertEqual(date_symbol_side_count(trades), 5)
        self.assertEqual(
            date_symbol_side_count(trades, direction="sell"), 2
        )
...
class CanonicalCliAndDataSourceTests(unittest.TestCase):
    """Verify the canonical CLI and explicit online/local source selection."""

    def test_canonical_module_cli_supports_help(self) -> None:
        completed = subprocess.run(
            [sys.executable, "-m", "quantfusion.application.backtest_cli", "--help"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_online_mode_uses_provider_failover(self) -> None:
        ...
        with (
            ...
            mock.patch.object(DataFetcher, "_fetch_tencent") as tencent,
        ):
            frame = DataFetcher.load_stock_data(
                "300308",
                "2026-01-02",
...
class RiskManagerTests(unittest.TestCase):
    ...
    def test_date_position_rejects_invalid_or_missing_dates(self) -> None:
        ...
        self.assertIsNone(
            RecoverableDrawdownRiskManager._date_position(
                "not-a-date",
                dates,
                positions,
            )
...
class ExecutionControlTests(unittest.TestCase):
    ...
    def test_signal_is_immutable(self) -> None:
        signal = Signal("300308", "alpha", "buy", target_shares=100)
    ...
    def test_fair_batch_allocation_is_order_independent(self) -> None:
        items = [
            (Signal("300308", name, "buy", target_shares=6_000), mock.Mock())
            for name in ("alpha", "beta")
        ]

        def allocate(batch: list[tuple[Signal, mock.Mock]]) -> dict[str, int]:
            shares = SleeveBacktestEngine._allocate_lots_pro_rata(batch, 6_000)
            return {
    ...
    @staticmethod
    def _sleeve(capital: float = 1_000_000) -> SleeveBacktestEngine:
        policy = PortfolioPolicy(allocation_mode="single")
        return SleeveBacktestEngine(
            capital,
            cfg=None,
            policy=policy,
    ...
    def test_batch_capacity_uses_the_strictest_strategy_exposure(self) -> None:
        ...
        items = [
            (
                Signal("300308", "turtle_breakout", "buy", 50_000, price=10.0),
                TurtleBreakoutStrategy(loose_cfg),
            ),
            (
                Signal("300308", "dual_ma", "buy", 50_000, price=10.0),
                DualMAStrategy(strict_cfg),
            ),
        ]
    ...
    def test_batch_rejects_mixed_symbols_or_execution_prices(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "exactly one symbol"):
            sleeve._buy_batch_capacity(
                [
                    (Signal("300308", "a", "buy", 100, price=10.0), strategy),
                    (Signal("300502", "b", "buy", 100, price=10.0), strategy),
                ],
                data_map,
                dates[-1],
            )
        with self.assertRaisesRegex(ValueError, "one execution price"):
            sleeve._buy_batch_capacity(
                [
                    (Signal("300308", "a", "buy", 100, price=10.0), strategy),
                    (Signal("300308", "b", "buy", 100, price=10.1), strategy),
                ],
                data_map,
                dates[-1],
            )
    ...
    def test_low_price_fee_floor_uses_exact_affordability_search(self) -> None:
        ...
        items = [
            (Signal("300308", name, "buy", target * 100), mock.Mock())
    ...
    def test_portfolio_liquidation_audits_a_superseded_sell_reason(self) -> None:
        ...
        BacktestEngine._apply_global_risk_lock(
    ...
    def test_synthetic_period_end_liquidation_is_not_configurable(self) -> None:
        with self.assertRaises(ValueError):
            BacktestEngine(cfg={"force_close_on_end": True})
...
class IntegrationTests(unittest.TestCase):
    ...
    def test_combined_same_day_fills_respect_portfolio_adv_budget(self) -> None:
        ...
        for (date_str, symbol, direction), shares in fills.items():
            ...
            history = history.loc[history.gt(0)].tail(PortfolioPolicy().adv_lookback)
            with self.subTest(date=date_str, symbol=symbol, direction=direction):
                self.assertFalse(history.empty)
                self.assertLessEqual(
                    shares / float(history.mean()),
                    PortfolioPolicy().max_order_adv_ratio + 1e-12,
                )
...
class NewFeatureTests(unittest.TestCase):
    """Verify current engine state and symbol accessors."""
...
class CrossSleeveExecutionIndependenceTests(unittest.TestCase):
    ...
    @staticmethod
    def _sleeve(name: str = "test") -> SleeveBacktestEngine:
        policy = PortfolioPolicy(allocation_mode="single")
        return SleeveBacktestEngine(
            1_000_000,
            cfg=None,
    ...
    @staticmethod
    def _states(
        *sleeves: SleeveBacktestEngine,
        pending: list[list[tuple[Signal, object]]],
    ) -> list[PreparedSleeveRun]:
        return [
            PreparedSleeveRun(
                sleeve=sleeve,
                data_map={},
    ...
    def test_open_execution_preserves_every_pending_signal(self) -> None:
        ...
        for case, orders in cases.items():
            with self.subTest(case=case):
                ...
                def execute_pending(
                    queued: list[tuple[Signal, object]],
                    _data_map: dict,
                    _date: pd.Timestamp,
                    _date_to_pos: dict,
                    allowed: frozenset[str],
                ) -> list[tuple[Signal, object]]:
                    executions.extend(
    ...
    def test_opposite_sleeves_create_real_two_sided_fills_and_costs(self) -> None:
        ...
        policy = PortfolioPolicy(allocation_mode="single")
        sell_sleeve = SleeveBacktestEngine(
            10_000.0,
            cfg=execution_cfg,
            policy=policy,
            allocation_lookbacks=policy.single_lookbacks,
        ...
        buy_sleeve = SleeveBacktestEngine(
            10_000.0,
            cfg=execution_cfg,
            policy=policy,
            allocation_lookbacks=policy.single_lookbacks,
        ...
        coordinator = BacktestEngine(20_000.0, cfg=execution_cfg)
