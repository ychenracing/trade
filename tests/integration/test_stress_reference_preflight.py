...
def test_missing_regime_reference_stops_before_provenance(tmp_path, monkeypatch):
    market = tmp_path / "market"
    regime = tmp_path / "regime"
    market.mkdir()
    regime.mkdir()
    # Regime referee is production front-5; omit one constituent to fail closed.
    omitted = "688256"
    assert omitted in stress.qf.PortfolioPolicy().regime_symbols
