...
def test_recovery_ownership_comes_only_from_same_day_filled_ab5_sells() -> None:
    ...
    sleeve_a = SimpleNamespace(
        trades=[
            TradeRecord("300308", "atr_channel", "sell", 200, 10.0, current, reason="account_budget_trim"),
            TradeRecord("300308", "dual_ma", "sell", 100, 10.0, current, reason="strategy exit"),
            TradeRecord("300502", "turtle_breakout", "sell", 100, 10.0, "2026-01-05", reason="account_budget_trim"),
        ]
    )
    sleeve_b = SimpleNamespace(
        trades=[
            TradeRecord("300394", "dual_ma", "sell", 300, 20.0, current, reason="account_budget_trim")
        ]
    )
...
def test_recovery_filter_blocks_only_owned_buy_books_and_never_sells() -> None:
    ...
    owned_sell = Signal(
        "300308", "atr_channel", "sell", target_shares=200, price=10.0,
        signal_date="2026-01-06", reason="risk exit",
    )
...
def test_hysteresis_enters_reduced_on_filled_trim_and_blocks_same_book_rebuy() -> None:
    ...
    sleeve = SimpleNamespace(
        trades=[TradeRecord("300308", "atr_channel", "sell", 200, 10.0, current, reason="account_budget_trim")],
        _c6_orders=None,
    )
    state = SimpleNamespace(sleeve=sleeve, pending=[(owned_buy, strategy), (unrelated_buy, strategy)])
...
def test_recovery_pending_keeps_owned_rebuy_blocked_for_second_safe_close() -> None:
    owned_buy = _buy()
    strategy = SimpleNamespace(name="atr_channel")
    state = SimpleNamespace(sleeve=SimpleNamespace(trades=[], _c6_orders=None), pending=[(owned_buy, strategy)])
    previous = _safe_envelope()
    previous.update(
        date="2026-01-05",
...
def test_account_budget_adapter_gates_before_canonical_and_annotates_envelope(monkeypatch) -> None:
    current = "2026-01-06"
    owned_buy = _buy()
    strategy = SimpleNamespace(name="atr_channel")
    sleeve = SimpleNamespace(
    ...
    apply_account_risk_budget_with_recovery(
        [state], pd.Timestamp(current), 100_000.0, 120_000.0,
        {"sentinel": True}, lambda _: 0.5, events, shock_floor=0.1,
    )
    envelope = events[-1]
