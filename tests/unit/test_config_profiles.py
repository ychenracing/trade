...
def _without_enabled_budget(config: Mapping[str, Any]) -> dict[str, Any]:
    # Pin the sole authorized default change while keeping every other field
    # under the original exact-content hashes; never regenerate those hashes.
    assert config["account_risk_budget_enabled"] is True
    return {key: value for key, value in config.items() if key != "account_risk_budget_enabled"}


def test_every_named_profile_keeps_exact_facts_with_budget_enabled() -> None:
    for name, expected in PROFILE_HASHES.items():
        assert _digest(_without_enabled_budget(getattr(profiles, name)())) == expected


def test_every_mapped_symbol_has_exact_effective_profile_at_default_shrinkage() -> None:
    effective = {
        code: _without_enabled_budget(profiles.config_for_symbol(code))
...
def test_default_config_content_and_validation_are_exact() -> None:
    ...
    assert _digest(_without_enabled_budget(defaults)) == (
