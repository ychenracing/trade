...
def test_selection_shape_preserves_deferred_base_predicates(branch):
    ...
    base_candidate, s_candidate = contract.preregistered_candidate_ids(p)
    ...
    state = {"branch": branch, "status": "selected" if selected else "rejected", "residual_ids": residuals,
             ...
             "selected_candidate": (base_candidate if branch == "BASE_SELECTED" else s_candidate) if selected else None,
