...
p.add_argument('--root', default='/mnt/data/trade')
...
fp = a._tree_fingerprint(a._source_files())
df = a._tree_fingerprint(a._data_files(MARKET_DATA_DIR, REGIME_DATA_DIR))
config = json.loads(args.cfg)
plan = s._multi_seed_scenarios(random_samples=50, permutation_samples=50, seeds=s.DEFAULT_SEEDS)
byid = {x['scenario_id']: x for x in plan}
ids = [sid for sid in args.ids if sid in byid]
assert len(ids) == len(set(args.ids))
...
if identity_path.exists() and json.loads(identity_path.read_text()) != identity:
    raise ValueError('Refuse mixed evidence identity')
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for future in as_completed(fs):
            try:
                ...
                print('FAILED', fs[future], repr(e), flush=True)
                raise
