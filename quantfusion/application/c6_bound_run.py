...
def execution_item_ids(
    ...
    if record_id == "c6.base.l1":
        ...
        scenario_path = Path(manifests["L1_ECONOMIC_SCENARIO_IDS"]["path"])
        scenarios = scenario_path.read_text(encoding="utf-8").splitlines()
        evaluations = [f"{binding['candidate_id']}::{scenario}" for scenario in scenarios]
...
def qualification_item_ids(export: RemoteExport) -> list[str]:
    ...
    base_ids = {
        ...
        and item.get("variant_id") in C6_CANDIDATE_SPECS
        and candidate_spec(str(item["variant_id"]))["role"] == "base"
    }
    if len(base_ids) != 1:
        _raise("Base producer candidate identity is ambiguous")
    base_id = next(iter(base_ids))
    selected = [
        {key: item[key] for key in ("scenario_id", "official_metrics")} for item in payload["evaluations"]
        if isinstance(item, dict) and item.get("variant_id") == base_id
    ]
...
def validate_checkpoint_item(item: Mapping[str, Any], prereg: Mapping[str, Any]) -> None:
    ...
    if name == "evaluation_record":
        ...
        met…6106 tokens truncated…       _raise(f"{label}: unsupported wire schema reference")
