...
class BacktestEngine:
    ...
    def _default_config() -> dict:
        ...
        return {
            ...
            "moat_weight_enabled": True,  # 护城河降权开关：弱护城河标的买入股数自动缩减。
            "moat_weight_min": 0.35,      # 护城河最低系数下限，防止过度压缩导致几乎无法建仓。
    ...
    def _fuse_daily_signals(
        ...
        for symbol, items in by_symbol.items():
            ...
            # 护城河降权：弱护城河标的买入目标股数额外缩减。
            moat_enabled = bool(self.cfg.get("moat_weight_enabled", False))
            moat_min = float(self.cfg.get("moat_weight_min", 0.35))
            if moat_enabled:
                raw_moat = MOAT_SCORE.get(symbol, 0.70)  # 未知标的默认中等偏弱
                moat_scale = max(raw_moat, moat_min)
            else:
                moat_scale = 1.0

            combined_scale = scale * moat_scale
            for signal, strategy in buys:
                signal.fusion_votes = votes
                signal.fusion_label = label
                signal.target_shares = _floor_to_lot(signal.target_shares * combined_scale)
                if moat_enabled and moat_scale < 1.0:
                    signal.reason = f"[{label}|护城河{moat_scale:.2f}] {signal.reason}"
                else:
            ...
            self.fusion_events.append({
                ...
                "buy_votes": votes, "sell_votes": 0,
                "scale": scale, "moat_scale": moat_scale,
...
# ── 护城河评分表 ──────────────────────────────────────────────────────
# 5档评分：1.0=全球龙头深护城河，0.85=国内绝对龙头，0.70=有壁垒但天花板有限，
#          0.55=护城河窄/龙头弱，0.40=蹭概念/非纯半导体业务。
# 买入信号目标股数 *= moat_score，弱护城河标的自动降仓。
# 评分依据：技术壁垒高度、全球/国内市场份额、客户认证深度、可替代性、业务纯度。
MOAT_SCORE: dict[str, float] = {
    # 第五梯队：全球龙头（1.0）
    "300308": 1.00,  # 中际旭创 — 光模块全球#1
    "300502": 1.00,  # 新易盛 — 光模块全球前三
    "300394": 1.00,  # 天孚通信 — 光器件精密制造龙头，深入英伟达供应链
    "688008": 1.00,  # 澜起科技 — DDR5接口芯片全球双寡头
    "688082": 1.00,  # 盛美上海 — 单片清洗设备全球技术领先
    # 第四梯队：国内绝对龙头（0.85~0.95）
    "688120": 0.95,  # 华海清科 — CMP设备国产>90%份额
    "688072": 0.90,  # 拓荆科技 — PECVD薄膜沉积国产#1
    "300666": 0.90,  # 江丰电子 — 靶材全球前列，进入台积电供应链
    "002409": 0.85,  # 雅克科技 — 前驱体国内领先，材料平台型
    "300604": 0.85,  # 长川科技 — 测试设备国产#1，市占10%→30%目标
    "603986": 0.90,  # 兆易创新 — NOR Flash全球#3+MCU国产#1
    # 第三梯队：有壁垒但天花板有限（0.70~0.80）
    "300054": 0.80,  # 鼎龙股份 — CMP抛光垫国产#1，打破陶氏垄断
    "688361": 0.75,  # 中科飞测 — 检测设备国产#1，但全球份额极小
    "688300": 0.70,  # 联瑞新材 — 硅微粉国内#1，天花板有限
    "688205": 0.70,  # 德科立 — 光通信第二梯队
    # 第二梯队：护城河窄/龙头弱（0.50~0.65）
    "688347": 0.65,  # 华虹宏力 — 代工全球#6-7，成熟制程
    "688249": 0.55,  # 晶合集成 — 代工全球#9，无先进制程
    "688409": 0.55,  # 富创精密 — 设备零部件=精密机加工，可替代性强
    "688535": 0.50,  # 华海诚科 — EMC低壁垒，并购驱动
    # 第一梯队：蹭概念/非纯半导体（0.35~0.45）
    "920045": 0.45,  # 蘅东光 — 无源光器件，低附加值走量型
    "600206": 0.40,  # 有研新材 — 杂货铺材料公司，靶材远落后江丰
    "300776": 0.35,  # 帝尔激光 — 98.6%营收来自光伏，半导体零收入
}
...
def main() -> dict | None:
    ...
    bt.add_argument("--no-moat", action="store_true", help="关闭护城河降权（用于A/B对比）")
    ...
    if args.command == "backtest":
        ...
        if args.no_moat:
            engine._user_cfg["moat_weight_enabled"] = False
        result = engine.run(
