...
当前版本：`1.0.3`。源码以 `main` 为唯一维护入口；功能、优势和使用限制见[发布说明](docs/RELEASE.md)。
...
当前版本已完成同一经济源码的 958 场景验证（经济源码 `9ff459d52c3daede7902a04b8861bc35eac53f92`，`economic_acceptance=MET`）。固定 17 股主池在 2025-04-01 至 2026-07-20、初始资金 200 万元及既定费用和滑点下，期末财富为初始本金的 9.610543 倍，最大回撤为 15.104978%。完整结果与复核入口见[验证结果与证据](docs/VALIDATION.md#formal-stress-evidence)。这些正式结果继续绑定原生产股票池和原窗口，不因研究股票池扩展而改写。
...
研究股票池 A–J 由名称映射为六位代码，不通过修改策略源码切换。`backtest_cli` 支持 `--pool`，日期参数统一为 `--start-date/--end-date`；默认起点为 `2023-01-01`。生产日扫仍固定使用 17 股生产池，不读取研究 Pool 配置。

市场数据下载器默认写入被忽略的 `data_cache/research_market`，不会覆盖 `data/market`。选择 Pool 时会取得所选 Pool 的并集、固定信号参考和现有独立风险篮；所有下载请求都在回放窗口前多取 365 个自然日用于 warm 指标。研究路径复用既有 Eastmoney→Sina→Tencent 提供方切换；若返回数据陈旧、Tencent 1000 行上限导致长历史明显截断、文件写入中断或全部提供方失败，则保存 `complete=false` 的 manifest 并失败关闭。Pool 比较强制要求 `complete=true` 的 manifest，并逐文件复核 SHA-256；缺失 manifest、身份不一致或文件漂移都会失败关闭。
...
日扫默认指数目录为独立运行目录 `data_cache/regime`，建议显式传入 `--regime-data-dir`，与股票缓存、输出目录一起固定运行位置。仓库冻结 `data/market`、`data/regime` 及已发布快照受写保护，不会被日扫刷新覆盖。冻结指数清单截至 2026-07-28，不能作为任意请求日的最新输入；历史目标仍需事先准备覆盖目标区间的合法指数数据。当前日决策消费既有 `detect_regime` 校验：固定指数证据为 `unknown`（不完整、无效或超过既有新鲜度容忍）时返回现金防御，不沿用非现金旧路由。该底层校验保留 `MAX_EVIDENCE_STALENESS_DAYS=10` 的自然日容忍；模拟日扫和账户建议入口还会依据独立沪深日历，强制核验应覆盖的交易日。日历仅覆盖 2024–2026 年，范围外或任一指数缺少目标交易日即失败关闭，不能靠共同陈旧或自然日容忍继续。输出分别记录请求日、应覆盖日、实际行情日和下一可交易日；数据来源及运行文件仍须真实合格。
...
机器信号先写，随后保存连续性状态；状态保存失败时 JSON 可能已经存在，命令仍返回失败。状态标记回写和成功索引更新失败也返回非零，不生成新阅读报告；已保存的风险状态不回滚。这不是多文件整体事务，读取索引仍须核对文件、日期和 run_id。身份不匹配时可以保留旧状态、发布禁止买入但仍保留卖出能力的结果；这与磁盘写入失败不是同一情况。详细顺序见[架构说明](docs/ARCHITECTURE.md)。
...
| 账户风险预算：合并回放、单账户回放、真实账户建议 | 同一账户连续高水位，预算基准为 `0.82 × peak`，另计两日规划压力与退出成本；公式唯一来源为 `risk/account_budget.py`。这不是保证实盘最大亏损 18%。 | 计算允许总敞口、裁剪买入并计划减仓；普通路径在超出不重大、剩余预算充裕且 `equity / peak ≤ 0.9245` 时可推迟按比例持仓减仓，硬路径不变。每次使用当时真实权益重新评价余额，未成交卖出不能增加买入额度，其他风险锁仍有效。真实账户使用可卖数量，回放使用既有成交队列。 |
...
| 正式验收指标：固定历史场景，不是交易触发器 | 按当前规则核验主池与各场景族的财富、回撤、排列和来源，交易统计用于成本分析。 | 判断验证结果是否满足发布要求，不直接产生买卖指令；完整规则见验证说明。 |
...
基础配置最大单股权重 60%、总仓位 100%、最多六只；实际额度还受行业画像、参考篮、市场状态、成交容量和账户预算的更严格限制。板块守卫使用组合政策中的固定 `regime_symbols` 五只温度计（可与交易池不一致）；大盘／科技路由使用固定指数 `000300` 与 `000682`。详见[架构说明](docs/ARCHITECTURE.md#市场状态路由与板块守卫温度计)。股票池扩展的固定参考、连续可执行意图确认、粘性候选与外围新标的准入由规范组合实现负责，不将简单的股票数量或当日排名当作充分买入依据。
...
底层回放在指数缺失、不可解析或某评价日证据不足时存在现金防御路径；当前日决策拒绝 `unknown` 指数证据。模拟日扫和账户建议入口另行强制校验请求应覆盖的交易日：任一必要指数缺少该交易日记录即失败关闭，自然日容忍也不能绕过该检查；不能把输入失败等同于已经生成有效现金建议。股票缺失处理仍按入口区分，见数据说明。
...
正式 CLI 默认使用 `native-default`，依据当前验收规则与固定 incumbent 比较。当前结果位于 `artifacts/validation/deployment-release/`，来源与复核方法见验证说明。诊断使用独立 checkpoint 和显式 diagnostic 输出；完整正式矩阵通过校验后才可发布 canonical 工件。
...
| `scripts/` | 批量验证、股票下载与研究工具；`download_market_data` 准备研究股票输入，`compare_universes` 运行跨 Pool 连续账户比较；统一用模块方式启动，先看各自 `--help`。 |
