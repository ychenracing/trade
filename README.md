...
本次策略改进的主要评估范围是固定17股科技主池。更换股票或使用任意子池时，需要重新评估；随机压力路径中存在明显收益退化，主池改善不能代替其他股票池的验证。具体部署验收与限制见[部署范围与限制](docs/VALIDATION.md#formal-stress-evidence)。
...
固定17股部署候选已完成同一源码958场景验证；扣除既定成本后，主池财富9.610543倍、最大回撤15.104978%，两者优于固定基准。随机子池仍有明显退化，最差仅保留基准约35.69%的财富；原合同仍未满足。完整当前结果、历史基线与使用限制见[验证结果与证据](docs/VALIDATION.md#formal-stress-evidence)。
...
正式 CLI 默认使用 `native-default`：同一候选依据冻结的部署范围合同与固定 incumbent 比较，原始财富参考和旧规则保留诊断，不能使用历史有限例外晋级。当前部署指标表与旧 accepted 基线工件分别标明身份，具体数值与来源见验证说明及原始证据。诊断写入独立 checkpoint 和显式 diagnostic 输出，不得搬入 canonical 路径冒充正式接受。仅在适用合同要求时运行未筛选的正式矩阵，不能把证据文件当作新的执行授权。
...
科技历史样本有幸存者偏差与事后关注偏差，前复权数据可能重述，日线模型无法证明盘中成交路径。历史基线接受包含明确的回撤、成交桶及财富保护例外，当前部署候选也不保证任意子池达标；默认预算在弱市研究对照中有机会成本，不能宣称全部窗口收益改善。日常指数使用独立运行目录，冻结输入受写保护；当前日决策拒绝 `unknown` 证据，入口另行强制校验交易日覆盖。日历范围有限，覆盖合格也不能替代预热、账户估值、风险状态及实际可执行性判断。
