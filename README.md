...
组合参数：`allocation_mode`、`single_lookbacks`、`allocation_horizons`、`drawdown_alert`、`confirmed_drawdown`、`drawdown_confirmations`、`emergency_drawdown`、`adv_lookback`、`max_order_adv_ratio`、`candidate_lookbacks`、`candidate_horizons`、`rearm_trading_days`、`terminal_drawdown`、`concentration_drawdown_adjustment`、`candidate_reference_percentile`、`drawdown_budget_enabled`、`drawdown_budget_peak_fraction`、`drawdown_budget_execution_buffer`、`drawdown_budget_adverse_atr_multiple`、`drawdown_budget_other_group_weight`、`drawdown_budget_release_ratio`、`drawdown_budget_reentry_days`、`drawdown_budget_recovery_confirmations`、`drawdown_budget_recovery_delta`、`drawdown_budget_normal_exit`、`regime_symbols`、`market_regime_enabled`、`regime_ewi_lookback`、`regime_breadth_ma_long`、`regime_adx_trend`、`regime_adx_choppy`、`regime_hurst_window`、`regime_hurst_trend`、`regime_hurst_choppy`、`regime_vol_lookback`、`regime_vol_extreme_pct`、`regime_ewi_slope_trend`、`regime_ewi_slope_choppy`、`regime_score_trend`、`regime_score_choppy`、`regime_choppy_confirmations`、`regime_trend_confirmations`、`regime_recovery_confirmations`、`regime_min_state_hold`、`regime_transition_scale`、`regime_trend_to_transition_confirmations`、`regime_choppy_exit_ratio`、`regime_transition_exit_ratio`。

`drawdown_budget_enabled` 默认关闭；当前只由压力测试入口对预注册的 C4 balanced/conservative 配置显式启用。L1 失败证据保留在 Draft PR，未被选为生产默认策略。
