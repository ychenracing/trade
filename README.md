...
`C6-Base+AB5` 在连续账户和原生订单队列上增加账户风险预算，必须显式设置
`account_risk_budget_enabled=True`；普通日扫、真实账户时点建议和未指定该配置的研究命令
不会自动获得这套连续账户预算。F0/F1/U 正确性修复属于共享生产路径，不等于自动启用 AB5。
...
`account_risk_budget_enabled` 默认启用；正常回测、生产逐日回放、日扫、单袖套及弱市路径
不需要额外参数。既有 AB5 预算从实际连续账户峰值形成收盘风险计划，通过原订单队列在后续
可交易开盘执行；不会因默认启用而改变 0.82 地板、账户锁、费用、手数或成交因果边界。
日扫工件的 `account_risk_budget` 包含实际执行次数和最后评估，不接受只有开关为真的成功结果。
历史消融必须显式指定关闭和原候选身份，不能冒充默认生产结果。

`--account` 默认把同一预算算法应用于真实快照建议，使用已要求的 `peak_equity`、现金和持仓。
它不把真实账户注入模拟历史，不伪造三个袖套；实际建议保留可卖数量、受阻数量及下一交易日
执行边界。必须提供同一账户、已按出入金一致处理的权益峰值；缺失或非有限峰值拒绝，缺行情
则明确标记预算未完成并抑制新增买入，不能静默关闭预算。实盘账户建议不是原 958 场景收益复现。
原已接受的回撤、财富保留和交易桶例外及历史来源见 [AB5 说明](docs/C6_AB5_RELEASE.md)。
