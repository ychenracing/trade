...
当前指数刷新会写入传入的 `regime_data_dir`，默认值仍指向冻结目录；日常命令应显式使用独立运行目录。状态机的 `boundary_route` 取最后一个可用指数日；`decide_current` 将既有 `detect_regime` 的 `unknown` 结果作为统一现金防御条件，旧路由不能覆盖无效当前证据。该检查不改变既有十个自然日的新鲜度容忍、状态机或历史回放；容忍范围内的证据不等于请求日完整性，仍应按数据说明核对实际覆盖。
