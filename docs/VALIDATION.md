...
与原已认证内容逐字节相同。五池共享引擎回归及历史未开启预算的研究结果不得冒充这份 AB5 正式结果。
这些固定历史样本不代表未来收益或任意股票池的表现。具体定义和来源见 `C6_AB5_RELEASE.md`。

## 默认启用预算的入口回归

当前黄金预期由同一已发布源码树的独立显式开启对照生成，再与新默认入口比较；
五池和两个自适应窗口的收益、回撤、成交及完整事件指纹逐项相同。复核运行使用 Python 3.13.5、
pandas 2.2.3、NumPy 2.3.5；这是同环境集成证据，不冒充原正式锁定环境或新的 958 验收。
确切输入、来源与逐项输出在 `tests/fixtures/account_budget_default_equivalence.json`；
原关闭预算的黄金值保留在 Git 历史，回归浮点容差、整数和指纹要求未放宽。
...
2024 年自适应弱市窗口在已有显式 AB5 与新默认 AB5 下均约为 +2.720470%，
旧关闭预算结果约为 +49.679035%；这是已有机制的机会成本，不能宣称默认开启必然增加收益。
真实账户建议与单账户/强制弱市入口有独立合成测试，不将这组回归扩大成所有入口的正式经济验收。

## 历史关闭预算的共享引擎回归

2026-09-11 的只读核对运行 `34631177857` 在相同锁定环境分别重放旧 main、冻结 I_B42
和发布集成源码。五池和单股自适应共六个用例的当前结果与冻结 I_B42 完全一致。
当时黄金预期从独立 I_B42 输出产生，精确指纹和浮点容差要求不变；旧黄金文件保留在旧 main。
