...
Pool A–J 与 2023 年起可配置窗口不修改本页既有来源绑定诊断。跨股票池连续账户比较使用 `python -m scripts.compare_universes --help`；股票池事实源为 `quantfusion/config/research_universes.py`，生产日扫的 17 股事实源仍为 `quantfusion/config/universe.py`。比较入口复用 `ProductionReplayEngine`，显式接受 `--start-date`、`--end-date`，默认比较 Pool B/F/G，也可重复传入 `--pool` 或使用 `--all-pools`。

研究比较在运行前要求所选股票池、固定信号参考、现有独立风险篮与两只固定指数证据完整；由研究下载器生成的完整 manifest 还须通过逐文件 SHA-256 核验。固定指数必须具有路由预热历史、窗口内一致日期覆盖和可接受的截止日新鲜度。缺失、截断、陈旧或哈希漂移均失败关闭，不能把输入不足造成的现金路径解释成策略主动防御。

比较报告分别记录请求窗口与权益曲线实际观察窗口，列出股票代码与名称、累计／年化收益、最大回撤、成交记录数、建模换手率、空仓比例、平均现金比例、按实际成交与截至当日最后可见收盘价重建的持仓 HHI，以及风险事件类型和次数。JSON 还保留股票 manifest 与固定指数证据身份。该报告是 `canonical=false` 的研究比较，不替代下文固定诊断或正式 17 股经济验收。

下文 `decision_diagnostics` 的固定 17 股、2025-04-01 至 2026-07-20 等范围是既有历史诊断身份的一部分，继续保留原值。需要研究新股票池或其他窗口时，不通过改写这些常量获得“新证据”。
...
生产账户输出 `candidate_diagnostics`，包括分数分项、确认数、排名、槽位选择、指示性目标数量及约束理由。评分、确认数目标权重和账户风险预算阈值均由生产路径决定，诊断只读取这些结果。股票评分能否改变有限槽位选择，与其是否能提高账户净收益，是两个不同问题。
...
`-no-budget` 仅是隔离研究调用的账户风险预算反事实，既不关闭日常风控，也不作为部署候选。基线和反事实的源码、数据、成本、初始资金、股票池、窗口及其他配置相同；汇总核对身份与对应运行条件，禁止将不同输入结果拼接比较。
...
预算约束的解除在当日收盘后才成为已知事实，因此恢复参与只从之后的交易日买入开始归因；同日开盘买入早于该收盘事实，不能算作“解除预算后恢复”。诊断遵守上述因果边界，不改变账户风险预算阈值、冷却或生产执行顺序。
...
评估核对账户代码、配置、日历及建议当时使用的行情前缀身份。原始快照、指数文件没有随评估输入提供时，明确保留“仅记录哈希、未重新核验原字节”的限制。代码、配置或历史前复权价格改变导致不匹配时输出 `IDENTITY_UNVERIFIED`，不使用不匹配的源码为已保存建议补发认证。应使用对应源码和原始冻结输入；不要篡改旧建议哈希来通过校验。
...
这些工件为 `canonical=false` 的诊断证据，不替代正式经济验收。所有对照保留失败、缺失和无法解释的风险变化；不改变黄金值、冻结输入、seed 或正式验收规则。
