...
`1.0.3` 在 `1.0.2` 基础上，收口普通路径账户风险预算（ordinary-AB5）的持仓减仓推迟规则，并刷新受影响的冻结黄金回归。固定 17 股生产池、既定风险阈值中的硬路径、交易费用、收盘后决策到下一可交易日执行边界，以及正式 958 场景的 incumbent 证据身份均未改写；正式经济验收未重新晋升。
...
- **正式 958 经济验收仍为 `NOT_MET`。** 黄金回归已按当前机制刷新，但正式 958 `economic_acceptance` 未重新跑通、未重新晋升；既有正式压力 incumbent 与发布回执身份保持不变，不能把本版描述为已通过正式 958 经济验收。
...
- 正式 958 场景计划、seed、incumbent 身份及既有发布回执；
...
冻结黄金回归已绑定当前 ordinary-AB5 持仓减仓推迟机制；Core17 与 `1.0.2` 一致，common-5 与部分研究池改善。正式 958 场景计划、seed 与既有 incumbent／发布回执身份仍保留，但本版正式 `economic_acceptance` 仍为 `NOT_MET`，未重新晋升。不得把冻结黄金刷新或诊断池改善表述为正式 958 已通过。完整正式证据入口见[验证说明](VALIDATION.md#formal-stress-evidence)。
